...
import 'package:SmartDeviceDart/features/smart_device/infrastructure/repositories/button_click/button_object_local_abstract.dart';
...
class ButtonObjectLocal extends ButtonObjectLocalAbstract {
  ...
  void buttonPressed(SmartDeviceBaseAbstract smartDevice,
    ...
    try {
      while (true) {
        var returnValue = await listenToButtonClick.listenToButtonPress(
            buttonPinNumber);
  ...
  void listenToButtonPressAndDoAction(SmartDeviceBaseAbstract smartDevice,
    ...
    while (true) {
      await listenToButtonClick.listenToButtonPress(buttonPinNumber).then((
          int exitCode) async {
